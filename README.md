...
## 制作首页App组件
1. 完成 Header 区域，使用的是 Mint-UI 中的Header组件   npm install mint-ui -S
...
### 制作顶部滑动条的坑们：
 ...
 4. 我们在初始化 滑动条 的时候，导入的 mui.js ，但是，控制台报错： `Uncaught TypeError: 'caller', 'callee', and 'arguments' properties may not be accessed on strict mode`
  ...
  + 最终，我们选择了 plan B  
     全局移除严格模式： 使用这个插件 babel-plugin-transform-remove-strict-mode
     忽略指定的文件 ：cli 中eslint忽略mui.min.js文件，在项目根目录添加 .eslintignore 
       ```
...
## 实现 图片详情中 缩略图的功能 
1. 使用 插件 vue-preview 这个缩略图插件  npm i vue-preview -S
